...
court_image = pygame.image.load('basketball_court.png')  # Make sure this path is correct
...
def main():
    ...
    while True:
        # Draw background (basketball court) only after the game starts
        if game_started:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if not in_game and 350 <= x <= 450 and 500 <= y <= 550:
                    ...
                    if 350 <= x <= 450 and 500 <= y <= 550:
                        game_started = True
                if game_started:
                    if 350 <= x <= 450 and 100 <= y <= 150:  # Score a point for the player
                        player_score += random.randint(1, 3)  # Random points between 1 and 3
                    elif 350 <= x <= 450 and 200 <= y <= 250:  # Opponent scores a point
        ...
        if not in_game:
            ...
            draw_text(f"{selected_player} - {selected_team}", font_large, TEXT_COLOR, (100, 50))
            draw_text(f"Score: {player_score}", font_large, TEXT_COLOR, (100, 150))
            draw_text(f"Opponent Score: {opponent_score}", font_large, TEXT_COLOR, (100, 250))

            # Display buttons for scoring
            draw_button("Score Point", font_small, BUTTON_COLOR, (350, 100), 100, 50)
            draw_button("Opponent Scores", font_small, BUTTON_COLOR, (350, 200), 100, 50)

            # Display win condition
            if player_score >= 21:
                draw_text("You Win!", font_large, TEXT_COLOR, (350, 300))
            elif opponent_score >= 21:
                draw_text("You Lose!", font_large, TEXT_COLOR, (350, 300))
